...
if (answer == "Yes" || answer == "yes")
{
    while (true)
    {
        ...
        if (trainer1.belt.Count <= 6)
        {
            for (int i = 0; i != 6; i++)
            {
                Console.WriteLine($"Give charmander {i+1} of Trainer {trainer1.name} a name : ");
                string charmanderName = Console.ReadLine();
                Console.WriteLine(trainer1.addPokeballsTotheBelt(charmanderName));
               ;
            }
        }
        ...
        if (trainer2.belt.Count <= 6)
        {
            for (int i = 0; i != 6; i++)
            {
                Console.WriteLine($"Give charmander {i+1} of Trainer {trainer2.name} a name : ");
                string charmanderName = Console.ReadLine();
                Console.WriteLine(trainer2.addPokeballsTotheBelt(charmanderName));
            }
        }
